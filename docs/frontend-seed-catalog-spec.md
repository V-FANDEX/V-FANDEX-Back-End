# 프론트엔드 연동 명세: 시즌 기본 장/종목 저장

## 1. 기능 목적

시즌 도중 관리자가 추가한 장과 새로 상장한 종목은 기본적으로 해당 시즌에만 존재하며, 시즌 초기화 시 삭제된다. 관리자가 장 또는 종목을 기본 카탈로그에 저장하면 다음 시즌 초기화에서도 유지된다.

기본 장과 종목은 다음 두 출처로 구분한다.

- `FILE`: `prisma/seed-data.example.json` 또는 운영용 로컬 시드 파일에서 생성된 장/종목
- `ADMIN`: 관리자가 API를 통해 기본 카탈로그로 승격한 장/종목
- `null`: 시즌 초기화 시 삭제되는 일반 장/종목
...
기존 `Market` 응답에 다음 필드가 추가된다. `GET /markets`, `GET /markets/:id`, `GET /admin/markets` 등에 동일하게 포함된다.
...
- `seedSource`: 장의 기본 카탈로그 저장 출처
- `seededAt`: 관리자가 마지막으로 기본 장으로 저장한 시각. 파일 시드 장은 `null`일 수 있다.
...
요청 본문은 사용하지 않는다. 성공 시 HTTP `200 OK`와 함께 갱신된 전체 `Market` 객체를 반환한다.
...
같은 장에 다시 호출할 수 있으며 `seededAt`이 최신 시각으로 갱신된다. 장을 저장해도 소속된 일반 종목이 자동으로 기본 종목이 되지는 않는다. 다음 시즌에도 유지할 종목은 각각 `POST /admin/stocks/:stockId/save-to-seed`로 저장해야 한다.

오류 응답:

- `401 Unauthorized`: 로그인 토큰 없음 또는 만료
- `403 Forbidden`: ADMIN 권한 없음
- `404 Not Found`: 존재하지 않는 `marketId`

## 5. 관리자 화면 권장 동작

### 장 목록

`GET /admin/markets?includeInactive=true` 결과의 `seedSource`를 사용해 다음 상태를 표시한다.

- `FILE`: `파일 기본 장`
- `ADMIN`: `관리자 저장 장`
- `null`: `시즌 한정 장`

`seedSource=null`인 장에는 `기본 장으로 저장` 명령을 제공한다. 저장 성공 후 응답 객체로 관리자 장 목록과 공개 장 목록 캐시를 갱신한다.

관리자 장 생성 화면에 `다음 시즌에도 유지` 체크박스를 둘 경우 다음 순서로 호출한다.

1. `POST /admin/markets`로 장 생성
2. 생성 응답의 `id`로 `POST /admin/markets/:id/save-to-seed` 호출

두 번째 요청이 실패하면 장 생성은 완료된 상태이므로 `장은 생성됐지만 기본 장 저장에 실패했습니다`라고 구분해 안내하고 재시도 기능을 제공한다.
...
- `adminSeedMarketsPreserved`: 관리자 저장 장 또는 관리자 저장 종목 때문에 유지된 파일 외 장 수
- `adminSeedStocksRestored`: 저장 가격으로 초기화된 관리자 기본 종목 수

초기화 완료 화면에서 파일 기본 장/종목 수와 관리자 저장 장/종목 수를 구분해 보여줄 수 있다.

## 7. 시즌 초기화 동작

관리자 기본 장은 시즌 초기화 시 다음과 같이 처리된다.

- 장 ID와 설정 유지
- 비활성 상태였더라도 다시 활성화
- 기본 장 안의 `seedSource=null` 종목은 삭제
...
- Market 타입에 `seedSource`, `seededAt` 추가
- 관리자 장 목록에 기본 장 상태와 저장 명령 추가
- `POST /admin/markets/:id/save-to-seed` API 함수 추가
- 신규 장 생성 후 선택적인 저장 API 연속 호출
